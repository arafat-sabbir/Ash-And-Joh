const Input = ({ name, type, onChange }) => {
  return <label className="relative">
    <input onChange={onChange} required placeholder="email" className="peer placeholder:text-transparent w-full px-3 text-xl py-2 bg-white border-2 border-black rounded-lg focus:outline-none transition-all duration-300 focus:border-2 " type={type} name={name} />
